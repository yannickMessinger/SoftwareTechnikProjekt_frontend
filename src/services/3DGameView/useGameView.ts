...
const gameState = reactive({ gameMapObjects: Array<IMapObject>(), mapObjsFromBackEnd:  Array<IMapObject>(), init: false});
...
export function fillGameState():void{
    ...
    for(let i = 0; i < mapWidth.value; i++){
        for (let j = 0; j < mapHeight.value; j++){
            ...
            gameState.gameMapObjects[counter]  = {objectTypeId:randomNumer(17,20), x: i, y: j, rotation:randomNumer(0,3)}
            counter +=1;
        }
    }
