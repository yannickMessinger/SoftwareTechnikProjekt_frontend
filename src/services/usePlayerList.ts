...
export async function fetchPlayerList(): Promise<void> {
    ...
    /* for (let i of result) {
        playerState.playerlist.push({
            userId: i.userId as IUser["userId"],
            userName: i.userName as IUser["userName"],
            errormessage: "" as IUser["errormessage"],
            loggedIn: true as IUser["loggedIn"],
            activeLobby: {
                lobbyId: -1,
                mapId: -1,
                lobbyName: "",
